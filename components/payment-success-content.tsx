...
export default function PaymentSuccessContent() {
  ...
  const coins = urlParams.get("coins"); // weekly, monthly, yearly
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      try {
        if (!userId || !coins || !amount || !method) {
          setStatus("❌ Missing required parameters");
          console.error("Missing parameters:", { userId, coins, amount, method });
          return;
        }
        ...
        if (method === "khalti" && pidx) {
          ...
          const res = await fetch("/api/khalti/verify", {
            ...
            body: JSON.stringify({ pidx, userId, coins, amount: parseInt(amount) }),
          });
    ...
    verifyPayment();
  }, [pidx, esewaResponse, userId, coins, amount, method]);
